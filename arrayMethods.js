...
// // CODE HERE
const postTaxPrices = prices.map( num => num * 1.07)// const postTaxPrices // = prices.map(/* Provide Your Callback Here );
...
// // CODE HERE
const totalPopulation = populations.reduce((a, c) => a + c) //  = populations.reduce(/* Provide Your Callback Here */)
...
const total = orders.map(order => order.price * (order.tax + 1))
...
const totalBob = purchases.filter(name => name.owner === "Bob").reduce((money, money2) => money + money2.price, 0)
